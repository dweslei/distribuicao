...
session_start();
if(!isset($_SESSION["usuario"]) || ($_SESSION["tipo"] != "beneficiario" && $_SESSION["tipo"] != "admin")){
...
<head>
  <meta charset="UTF-8">
  <title>Beneficiário - Solicitar Alimentos</title>
...
<div class="d-flex">
  ...
  <div class="content flex-grow-1 p-4">
    ...
    <p class="lead">Aqui você pode solicitar alimentos disponíveis.</p>

    <div class="card p-4 shadow-sm mt-3">
      <form method="POST">
        <div class="mb-3">
          <label class="form-label">Alimento desejado</label>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $local = $_POST["local"];
        $beneficiario = $_SESSION["usuario"];

        $sql = "INSERT INTO solicitacoes (beneficiario, alimento, quantidade, local) 
                VALUES ('$beneficiario','$alimento','$quantidade','$local')";
        if ($conn->query($sql) === TRUE) {
            echo "<div class='alert alert-success mt-3'>Solicitação registrada com sucesso!</div>";
        } else {

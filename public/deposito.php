...
session_start();
if(!isset($_SESSION["usuario"]) || ($_SESSION["tipo"] != "deposito" && $_SESSION["tipo"] != "admin")){
...
<div class="d-flex">
  ...
  <div class="content flex-grow-1 p-4">
    ...
    <!-- Formulário para adicionar item ao estoque -->
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $deposito = $_SESSION["usuario"];

        $sql = "INSERT INTO estoque (deposito, alimento, quantidade) 
                VALUES ('$deposito','$alimento','$quantidade')";
        if ($conn->query($sql) === TRUE) {
            echo "<div class='alert alert-success mt-3'>Item adicionado ao estoque!</div>";
        } else {
    ...
    // Listagem do estoque
    $sql = "SELECT * FROM estoque ORDER BY data DESC";
    $result = $conn->query($sql);
    if ($result && $result->num_rows > 0) {
        echo "<div class='card p-3 shadow-sm mt-4'>";
        echo "<h5>📋 Estoque Atual</h5>";
        echo "<table class='table table-striped'>";
        echo "<thead><tr><th>Alimento</th><th>Quantidade</th><th>Data</th></tr></thead><tbody>";
        while($row = $result->fetch_assoc()) {
            echo "<tr><td>".$row["alimento"]."</td><td>".$row["quantidade"]."</td><td>".$row["data"]."</td></tr>";
        }
